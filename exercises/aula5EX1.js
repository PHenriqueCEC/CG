...
const lerpConfig = {
  destination: new THREE.Vector3(-4.5, 0.2, 3.0),
  alpha: 0.02,
  move: true
  //reset: false
}
...
/*     folder.add(lerpConfig.destination, 'y', 0.1, 3).onChange();
    ...
    folder.add(lerpConfig, 'alpha', 0.02, 1).onChange();      
    folder.add(lerpConfig, "move",  true)
          .name("Move Object");

    folder.add(lerpConfig, 'reset', false).name("reset")
}
